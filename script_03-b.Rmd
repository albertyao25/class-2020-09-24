...
# it appears that the biggest drops are due to heavy workload, such as Gov 50.
# Lots of Geneds were also dropped

course_10 %>%
  select(id, title.new, department.new, diff) %>%
  ggplot(aes(title.new, diff)) +
  geom_col() +
  coord_flip()
